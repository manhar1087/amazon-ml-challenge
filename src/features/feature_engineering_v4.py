...
def partial_ratio(df, col1, col2):
    c1 = df[col1].to_list()
    c2 = df[col2].to_list()
    res = [fuzz.partial_ratio(str(x) if x else "", str(y) if y else "") / 100.0 if x is not None and y is not None else 0.0 for x, y in zip(c1, c2)]
    return pl.Series(res, dtype=pl.Float32)

def numeric_overlap(df, col1, col2):
    c1 = df[col1].to_list()
    c2 = df[col2].to_list()
    res = []
    for x, y in zip(c1, c2):
        n1 = set(re.findall(r'\d+', str(x))) if x else set()
        n2 = set(re.findall(r'\d+', str(y))) if y else set()
        if not n1 and not n2:
            res.append(1.0)
        elif not n1 or not n2:
            res.append(0.0)
        else:
            res.append(len(n1 & n2) / len(n1 | n2))
    return pl.Series(res, dtype=pl.Float32)

def build_features_v4(pairs_lf: pl.LazyFrame, s1_lf: pl.LazyFrame, s23_lf: pl.LazyFrame) -> pl.DataFrame:
    ...
    df = df.with_columns([
        ...
        partial_ratio(df, "latin_name", "latin_name_right").alias("f_name_partial_ratio"),
        numeric_overlap(df, "latin_name", "latin_name_right").alias("f_name_num_overlap"),
        ...
        partial_ratio(df, "normalized_address", "normalized_address_right").alias("f_addr_partial_ratio"),
        numeric_overlap(df, "normalized_address", "normalized_address_right").alias("f_addr_num_overlap")
    ])
    
    # V4 Targeted Rescues based on V3 Error Analysis
    df = df.with_columns([
        ...
        # 1. Postal Rescue: Over 90% of FNs had a postal match. If postal matches, house doesn't conflict, and name has strong subset overlap, rescue it.
        ((pl.col("f_postal_match") == 1.0) & (pl.col("f_house_conflict") == 0.0) & (pl.col("f_name_set_ratio") > 0.8)).cast(pl.Float32).alias("f_rescue_postal_name_set"),
        
        # 2. Exact Name Rescue: 1.5% of exact names were FNs. If name is exact and geography doesn't explicitly conflict, rescue it.
        ((pl.col("f_name_exact") == 1.0) & (pl.col("f_postal_conflict") == 0.0) & (pl.col("f_house_conflict") == 0.0)).cast(pl.Float32).alias("f_rescue_exact_name_safe"),
        
        # 3. Source-specific tuning: Interactions to allow the model to lower thresholds for S3 specifically (since S3 FN rate is 2x S2).
        (pl.col("f_is_s3") * pl.col("f_name_sort_ratio")).alias("f_s3_name_sort"),
        (pl.col("f_is_us") * pl.col("f_addr_num_overlap")).alias("f_us_addr_num")
    ])
