...
def slice_v3_tune_errors():
    print("Loading V3 raw TUNE predictions...")
    preds = pl.read_parquet("work/task06/v3_raw_tune_predictions.parquet")
    tune_ids = preds["source1_entity_id"].unique().to_list()
    
    print(f"Tuning S1s in predictions: {len(tune_ids)}")
    ...
    for row in cands_df.select(["source1_entity_id", "candidate_entity_id"]).iter_rows():
        s_id, c_id = row
        if s_id in tune_ids and c_id is not None:
    ...
    print(f"\n--- Candidate-Limited Oracle (TUNING SET) ---")
    print(f"Tune Total GT: {total_gt}")
    print(f"Tune Retrieved GT: {retrieved_gt}")
    ...
    with open("work/task06/v3_tune_error_analysis.json", "w") as f:
        json.dump(analysis, f, indent=2)
        
    print("V3 Tuning Error analysis completed and saved to work/task06/v3_tune_error_analysis.json")

if __name__ == "__main__":
    slice_v3_tune_errors()
